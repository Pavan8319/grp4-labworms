...
export const getUserInfo = async (email, password) => {
  ...
  // Create auth mapping using modular API
export const createAuthMapping = async (phoneAuthUid, emailAuthUid) => {
  try {
      // Create mappings in both directions
      await setDoc(doc(db, 'authMappings', phoneAuthUid), {
          emailAuthUid: emailAuthUid,
          createdAt: new Date().toISOString()
      });

      await setDoc(doc(db, 'authMappings', emailAuthUid), {
...
export const register = async (userData) => {
  try {
      ...
      // Store the phone auth UID before creating email auth
      const phoneAuthUid = auth.currentUser?.uid;
      ...
      const emailAuthUser = userCredential.user;
      
      // Create mapping between phone and email auth UIDs
      if (phoneAuthUid) {
          await createAuthMapping(phoneAuthUid, emailAuthUser.uid);
      }

      // Send email verification
      await sendEmailVerification(emailAuthUser);

      // Get the ID token
      const idToken = await emailAuthUser.getIdToken();

      // Prepare user data
      const userDocData = {
          uid: emailAuthUser.uid,
          phoneAuthUid: phoneAuthUid, // Include phone auth UID
      ...
      await setDoc(doc(db, 'users', emailAuthUser.uid), userDocData);
...
export const sendEmailOTP = async (email) => {
    try {
        ...
        const response = await fetch(`${apiUrl}/api/check-email-exists`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        // Send OTP email
        const otpResponse = await fetch(`${apiUrl}/api/send-email-otp`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
